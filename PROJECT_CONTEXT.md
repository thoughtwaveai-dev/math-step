...
9. **Password reset**: `/login` → "Forgot password?" → `/account/forgot-password` (server action `requestPasswordReset` → `supabase.auth.resetPasswordForEmail` with `redirectTo` set to `${NEXT_PUBLIC_APP_URL ?? 'https://mathstep.nz'}/auth/callback?next=/account/update-password`). Email link lands on `/auth/callback`, whose route handler accepts **either** `?token_hash=…&type=recovery` (verified with `supabase.auth.verifyOtp`, the current path) **or** a legacy `?code=` (exchanged with `exchangeCodeForSession`, retained only for links already sitting in inboxes) → redirects to `/account/update-password` (server-side `getUser()` gate → client form → `updatePassword` server action calls `supabase.auth.updateUser({ password })`) → `/login?reset=1` with success banner. Supabase Auth → URL Configuration → Redirect URLs must include `https://mathstep.nz/auth/callback` (plus `http://localhost:3000/auth/callback` for dev). **The Recovery email template is customised (2026-08-16) and must stay that way**: its link is `https://mathstep.nz/auth/callback?token_hash={{ .TokenHash }}&type=recovery&next=/account/update-password`. Reverting it to the stock `{{ .ConfirmationURL }}` reintroduces the cross-device bug described below, because that form produces a PKCE `?code=` link that only verifies in the browser which requested the reset. Domain is hardcoded rather than `{{ .SiteURL }}` so the link cannot break if Site URL is ever changed. Still no custom token logic and no Resend wiring in app code; Supabase's built-in email service sends it.

**Send-failure visibility (2026-08-16).** `requestPasswordReset` returns `{ sent: boolean; error?: string }`. It defaults to `{ sent: true }` whatever happens, because always reporting success is what stops the page being used to test which families have a MathStep account. It reports a failure **only** when the Supabase error code is in the closed allowlist `SEND_INFRASTRUCTURE_ERROR_CODES` (`unexpected_failure`, `request_timeout`, `over_request_rate_limit`, `email_address_not_authorized`), or when the error carries no code at all and no status (or status `0`), which means the request never reached Supabase. Every other code stays silent, including `user_not_found`, `user_banned`, `email_not_confirmed`, and anything a future Supabase release adds. The allowlist is deliberately closed so a new account-specific code cannot leak by default. **Do not convert this to a denylist, and do not add per-cause messages**: all reported failures share one neutral string (*"Something went wrong on our end. Please try again in a few minutes."*) so the presence of an error is the only signal, never its wording. Supabase's own enumeration protection is also on, so a reset for a non-existent address returns no error at all; the allowlist is defence in depth. Errors are logged as a flat interpolated string with `code` and `status` (an object argument gets serialised to `{}` by the Next dev logger).

**`over_email_send_rate_limit` must never be added to the allowlist.** It was on it briefly on 2026-08-16 and was removed the same day after the audit tested it against the live project: a real account returns HTTP 429 on a second request inside 60 seconds, while an address with no account returns HTTP 200 every time, so reporting that code answers "does this account exist?" for anyone who submits an address twice. The project-wide email quota returns the same code but never fired even under rapid sends, so on this project the code means the per-address cooldown and nothing else. There is also nothing to report: the cooldown only fires when a reset email was successfully sent seconds earlier, which makes "Check your inbox" the truthful answer. Accepted residual: if the project-wide quota is ever exhausted it will go silent, which is tolerable because genuine SMTP failures surface as `unexpected_failure`.

**Cross-device reset bug (fixed 2026-08-16, commit `8bda7fe`).** Recovery links are PKCE tokens. The old callback only handled `?code=` via `exchangeCodeForSession`, which requires the `sb-<ref>-auth-token-code-verifier` cookie set in the browser that requested the reset. Opening the email on a different device (phone vs laptop) therefore always failed to `/account/forgot-password?error=expired`, and requesting a new link produced the same result every time, so affected parents could never recover their account. Verified in production: cookie-less client got `303 → /auth/callback?code=… → 307 → ?error=expired`, while the same link in the originating browser reached `/account/update-password`. `verifyOtp` carries no browser-bound state, so the `token_hash` path works from any device (confirmed against a `pkce_`-prefixed token with zero cookies present).
...
| 14/1 | inequalities (5 types: `inequality_one_step`, `inequality_two_step`, `inequality_negative_coefficient`, `inequality_check_value`, `inequality_from_words`) | `"x < 6"` / `"x <= 4"` / `"x < -4"` (solve+words) / `"yes"` / `"no"` (check) | inequality path (`/[<>]/`, normalizes `≤`/`≥` ↔ `<=`/`>=`, spacing/case) for solve+words / algebraic path for yes/no. One-variable, integer-only, no inequality graphing in v1. **No `gradeAnswer` change.** Extends `inequalities.ts` (shared with 11.1) — 6-member `InequalityProblemType` union + `generateInequalitiesLevel141`, id prefix `ineq141_`. Reuses the shared answer-control system: check → `yes_no` buttons, the other four → default text input (`inputMode="text"`, `e.g. x < 6` / `e.g. x + 5 <= 12`). Solve/words prompts append a generic format hint (number ≠ answer). |
| 14/2 | exponents (5 types: `exponent_evaluate`, `exponent_multiply_same_base`, `exponent_divide_same_base`, `exponent_power_of_power`, `exponent_zero_and_one`) | plain non-negative integer: `"81"`, `"7"`, `"1"` | signed-integer path (`/^-?\d+$/`). **No `gradeAnswer` change.** Generator `src/lib/math/generators/exponents.ts`, id prefix `exp142_`. Prompts render powers as Unicode superscripts (`3⁴`, `x⁵ × x⁶`) so they read like a textbook; `sup()` maps digits to `⁰-⁹`. The three index-law types ask for the resulting **power as a number** rather than the expression `x⁷` — keeps the law itself as the thing tested while avoiding caret/superscript typing on a tablet. `exponent_evaluate` bounds `base^exp ≤ 1000` via `maxExponentFor()`. Divide type forces `a > b` so results are never zero or negative powers. All types use the default numeric input (`inputMode="numeric"`). |
...
| 17/1 | solving quadratics by factorising (5 types: `solve_quadratic_factorised`, `solve_quadratic_positive_roots`, `solve_quadratic_negative_roots`, `solve_quadratic_mixed_roots`, `solve_quadratic_difference_of_squares`) | two roots, smaller first: `"x = -5 or x = 3"`, `"x = 2 or x = 7"` | algebraic path (`/[a-zA-Z]/`, lowercase + strip whitespace, strict match; no `y=` so the sim-eq branch never fires). **No `gradeAnswer` change.** Generator `src/lib/math/generators/solving-quadratics.ts`, id prefix `sq171_`. Every maker picks the two roots first (always distinct, nonzero integers, magnitude 2 to 9, or 2 to 10 for difference of squares) and derives the equation, reusing `formatQuadratic` from 16.1 for the prompt. Mixed type keeps the root sum at least 2 away from 0 (40 retries, fixed fallback) so no `0x` or `1x` term appears. The strict grader does not reorder, so the generator exports `formatRoots(r1, r2)` (sorts ascending) as the single source of truth, imported by the new **`quadratic_roots` answer control** so the student can enter the roots in either order. All five types use `inputMode="text"` (stylus-bug rule) for the fallback input; placeholder `e.g. x = -1 or x = 12` can never be a real answer. |
| 17/2 | sequences (5 types: `sequence_next_term`, `sequence_missing_term`, `sequence_nth_term_value`, `sequence_find_nth_term`, `sequence_term_position`) | positive integer for four types (`"28"`, `"12"`); nth term rule for `sequence_find_nth_term` (`"3n + 4"`, `"5n - 2"`) | signed integer path for four types; algebraic path (letter n, lowercase + strip whitespace, strict match) for `sequence_find_nth_term`. **No `gradeAnswer` change.** Generator `src/lib/math/generators/sequences.ts`, id prefix `seq172_`. Every maker picks the sequence or the rule and position first, so answers are exact by construction. Decreasing sequences pick the last term first (1 to 30), so every shown term and answer is >= 1. Rules use a 2 to 9 and b -9 to 9 excluding 0; find_nth_term also needs a + b >= 1. find_nth_term prompts end with the fixed hint "Write it like 10n + 11." and use the placeholder `e.g. 10n + 11`; a is never 10 and b is never 11, so no part of either can match a real answer. `inputMode="text"` for find_nth_term (stylus-bug rule), `numeric` for the other four. Default answer control for all five. |
| 18/1 | Pythagoras' Theorem (5 types: `pythagoras_hypotenuse`, `pythagoras_shorter_side`, `pythagoras_check_right_angle`, `pythagoras_word_problem`, `pythagoras_distance_points`) | plain positive integer `"13"`, `"24"` / `"yes"` / `"no"` (check) | signed-integer path (`/^-?\d+$/`) for the four numeric types / algebraic path for yes/no. **No `gradeAnswer` change.** Generator `src/lib/math/generators/pythagoras.ts`, id prefix `pyt181_`. Text only, no diagrams. Every number comes from a fixed table of 7 primitive triples and their whole-number multiples (hypotenuse <= 65), so answers are whole by construction. Check type: 2 yes / 2 no per 20; a "no" nudges one side of a triple by 1 and is rejected if it is still a triple, is not a triangle, or has two equal sides; sides shown sorted or shuffled. Ladders capped at 13 m. Distance: legs <= 30, coordinates -10 to 20, a coordinate equal to the distance is rejected. Check rides the `yes_no` control (`inputMode="text"`); the other four use the default numeric input with placeholder `e.g. 23` (never a side of any triple here, so it cannot leak an answer). |
| 18/2 | ratio and proportion (5 types: `ratio_share_larger`, `ratio_share_smaller`, `ratio_missing_value`, `ratio_find_total`, `proportion_unitary`) | single positive integer: `"35"`, `"3"`, `"154"` | signed-integer path (`/^-?\d+$/`). **No `gradeAnswer` change.** Generator `src/lib/math/generators/ratio.ts`, id prefix `rat182_`. An answer is never a ratio string, because the grader's digit-set fallback would accept `"4:3"` for `"3:4"`. Ratios are in simplest form (a != b, 1 to 9, coprime) and each maker picks the ratio and the value of one part first, so answers are whole by construction. Missing-value hides any of the four positions; find-total uses sweets, stickers or money; unitary uses shop prices, recipe grams, car km and printer pages. The build loop rejects a draw whose prompt shows the answer as a number. Default control, `inputMode="numeric"`, placeholder `e.g. 37` (a prime above 12, which no answer can be). Prompts with $, g or km end with "Answer with just the number." |
| 19/1 | percentage change (5 types: `percent_increase_amount`, `percent_decrease_amount`, `percent_change_find`, `percent_reverse`, `percent_multiplier`) | positive integer: `"92"`, `"156"`, `"25"` (types 1 to 4) / decimal: `"1.12"`, `"0.7"` (multiplier) | signed-integer path (`/^-?\d+$/`) for types 1 to 4 / decimal path (`/^\d+\.\d+$/`, tolerance 0.001, so `"1.50"` and `"0.70"` still grade correct) for the multiplier. **No `gradeAnswer` change.** Generator `src/lib/math/generators/percentage-change.ts`, id prefix `pct191_`. Builds on 6.2 (whose type names it does not reuse). Integer maths only: every base is a round multiple of `100 / gcd(p, 100)` so p% of it is whole, and the multiplier is formatted from the whole number of hundredths (100 + p or 100 - p) by string logic, never `toFixed`. Guards: amount answers are always 20 or more and percent-change answers are multiples of 5 from 5 to 90, so the `e.g. 13` placeholder can never be a real answer; multiplier percents run 5 to 95, so `e.g. 2.5` can never be one; the build loop rejects any prompt that shows its own answer. Types 1 to 4 use `inputMode="numeric"` and end with "Answer with just the number."; the multiplier uses `inputMode="decimal"`. Default control for all five. Smoke `scripts/level-19-1-smoke.ts`. |
| 19/2 | area and perimeter (5 types: `area_triangle`, `area_parallelogram`, `area_trapezium`, `area_compound_shape`, `rectangle_perimeter_area`) | single positive integer: `"40"`, `"68"` | signed-integer path (`/^-?\d+$/`). **No `gradeAnswer` change.** Generator `src/lib/math/generators/area-perimeter.ts`, id prefix `ap192_`. Text only: no diagrams, no circles. Lengths are picked first; triangle and trapezium bump one length by 1 when the product being halved would be odd, so answers are whole by construction. Parallelogram carries a slanted-side distractor (longer than the height, never equal to the base, sentence order of height and slant varies). Compound cut-out is strictly smaller than the rectangle both ways. Rectangle type alternates area-to-perimeter and perimeter-to-area (2 + 2 per worksheet), length > width, 6 by 3 skipped (area = perimeter = 18). Build loop rejects any prompt that shows its own answer. Units cm or m, squares as `cm²`; every prompt ends "Answer with just the number." All five use `inputMode="numeric"` and the default control; placeholders `e.g. 74` / `e.g. 24` are unreachable answers. |
...
`src/lib/lessons/index.ts` — static content keyed by `"level/sublevel"`. All 38 currently supported levels have lesson cards: 1/1, 1/2, 2/1, 2/2, 3/1, 3/2, 4/1, 4/2, 5/1, 5/2, 6/1, 6/2, 7/1, 7/2, 8/1, 8/2, 9/1, 9/2, 10/1, 10/2, 11/1, 11/2, 12/1, 12/2, 13/1, 13/2, 14/1, 14/2, 15/1, 15/2, 16/1, 16/2, 17/1, 17/2, 18/1, 18/2, 19/1, 19/2.
...
- **Mapping:** `src/lib/math/answerControl.ts → getAnswerControlType(type)` returns one of
  `equation_slope_intercept` | `quadratic_expression` | `bracket_pair` | `quadratic_roots` | `yes_no` | `coordinate_pair` | `default`.
  ...
  - Level 17.1 `solve_quadratic_*` (5 types) → quadratic roots control (`x = [±][a] or x = [±][b]`, canonical `x = -5 or x = 3`)
  - `point_on_line`, `system_check_solution`, `inequality_check_value`, `pythagoras_check_right_angle` → Yes/No buttons (canonical `yes`/`no`)
...
- **Components:** `src/components/answer-controls/` — `AnswerInput.tsx` (dispatcher),
  `EquationSlopeInterceptInput.tsx`, `QuadraticExpressionInput.tsx`, `BracketPairInput.tsx`, `QuadraticRootsInput.tsx`, `CoordinatePairInput.tsx`,
...
- **Quadratic control rules (2026-09-08):** the `x²` is printed by the control, never typed, so the
  superscript is unreachable from a tablet keyboard. Only the two signs and the two magnitudes are
  student input. A blank field, or a zero `x` coefficient, counts as "no answer" (the 16.1 generator
  never emits a zero `x` term), matching the equation control. The canonical string comes from the
...
- **Quadratic roots control rules (2026-09-25):** same idea as the bracket pair control. The
  answer `x = -5 or x = 3` rides the strict algebraic path, so the control builds it through the
  17.1 generator's own `formatRoots(a, b)`, which puts the smaller root first. Only a blank field is
  "no answer"; a 0 passes through (never as `-0`) and simply grades wrong, since 17.1 never has a 0
  root. `scripts/answer-control-gate.ts` proves both entry orders build the same string and grade
  `=== true` for every distinct pair from -10 to 10 (excluding -1, 0, 1).
...
- `SUPPORTED_LEVEL_KEYS` — ordered list of all levels with generator support: `[1,1],[1,2],[2,1],[2,2],[3,1],[3,2],[4,1],[4,2],[5,1],[5,2],[6,1],[6,2],[7,1],[7,2],[8,1],[8,2],[9,1],[9,2],[10,1],[10,2],[11,1],[11,2],[12,1],[12,2],[13,1],[13,2],[14,1],[14,2],[15,1],[15,2],[16,1],[16,2],[17,1],[17,2],[18,1],[18,2],[19,1],[19,2]`
